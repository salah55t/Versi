"""
╔══════════════════════════════════════════════════════════════════════════════╗
║           🔥 CYBERPUNK DISTRIBUTOR CORE v5.1 PRO EDITION 🔥                 ║
║                                                                              ║
...
logger.info("╔═══════════════════════════════════════════════════════════════╗")
logger.info("║  🔥 CYBERPUNK DISTRIBUTOR CORE v5.1 PRO - INITIALIZING... 🔥  ║")
logger.info("╚═══════════════════════════════════════════════════════════════╝")
...
class DeliveredAccount(Base):
    ...
    delivery_type = Column(String(20), default="claim")  # claim, reservation
...
def log_audit(db, user_id: Optional[str], action: str, details: str, request: Request = None):
    """Log action to audit trail - safely handles None request"""
    try:
        ip = None
        ua = None
...
def clean_expired_reservations(db):
    """Remove expired reservations and free accounts - FIXED"""
    now = datetime.datetime.utcnow()
    ...
    for res in expired:
        res.status = "expired"
        # FIXED: Free the account back to inventory
    ...
    if count > 0:
        db.commit()
        logger.info(f"🧹 Cleaned {count} expired reservations and freed accounts")
    return count


# ═══════════════════════════════════════════════════════════
#                    PENDING ACTIONS (MOVED UP)
# ═══════════════════════════════════════════════════════════
# FIXED: These must be defined BEFORE the telegram_webhook handler
...
async def tg_send_document(chat_id: str, file_path: str, caption: str = ""):
    """FIXED: Use run_in_executor to avoid blocking the event loop"""
...
async def receive_hit(request: Request):
    with get_db() as db:
        try:
            ...
            # FIXED: Extract all data first before any checks
            config_name = "UNKNOWN"
            ...
            # FIXED: Handle IntegrityError for race conditions
            try:
                db.add(Account(
...
async def telegram_webhook(request: Request):
    # FIXED: Use context manager for safe session handling
    with get_db() as db:
        try:
            ...
            if "callback_query" in payload:
                ...
                if data.startswith("claim_cfg:"):
                    ...
                    # FIXED: Check cooldown only for actual claims (not reservations)
                ...
                if data.startswith("cancel_reserve:"):
                    ...
                    # FIXED: Only search in active reservations
                    ...
                    if reservation:
                        ...
                        if selected and reservation.config_name != selected:
                            await tg_answer(callback_id, "❌ الحجز غير مطابق.", show_alert=True)
                            return {"status": "ok"}
                        ...
                        db.commit()
                        log_audit(db, chat_id, "reservation_cancelled", f"Cancelled reservation for {reservation.config_name}", request)
            ...
            # ── Claim Account ──
                ...
                # FIXED: Check cooldown only for actual claims
                last_delivery = db.query(DeliveredAccount).filter(
                    DeliveredAccount.user_id == chat_id,
                    DeliveredAccount.delivery_type == "claim"
                ).order_by(desc(DeliveredAccount.delivered_at)).first()
...
async def export_and_send(db, chat_id: str, fmt: str = "json"):
    ...
    tmp_dir = tempfile.gettempdir()  # FIXED: Use proper temp directory
    ...
    if fmt == "json":
        ...
        filepath = os.path.join(tmp_dir, f"accounts_export_{timestamp}.json")
        ...
        import csv
        accounts = db.query(Account).all()
        filepath = os.path.join(tmp_dir, f"accounts_export_{timestamp}.csv")
...
async def export_audit_log(db, chat_id: str):
    ...
    filepath = os.path.join(tmp_dir, f"audit_log_{timestamp}.json")
    data = []
    for log in logs:
        data.append({
...
async def create_full_backup(db, chat_id: str):
    ...
    filepath = os.path.join(tmp_dir, f"full_backup_{timestamp}.json")
    with open(filepath, "w", encoding="utf-8") as f:
...
async def admin_ban_user(ban_req: BanRequest, request: Request):
    ...
    with get_db() as db:
        ...
        # FIXED: Set banned_by from request or use system
        banned_by = "system"
        try:
            if request and request.client:
                banned_by = request.client.host or "system"
        except:
            pass
...
async def admin_dashboard():
    ...
    with get_db() as db:
        ...
        claims_week = db.query(DeliveredAccount).filter(
            DeliveredAccount.delivered_at >= week_ago,
            DeliveredAccount.delivery_type == "claim"
        ).count()
...
async def periodic_cleanup():
    ...
    while True:
        try:
            ...
            logger.error(f"Periodic cleanup error ({consecutive_errors}/{max_consecutive_errors}): {e}")
            if consecutive_errors >= max_consecutive_errors:
                logger.critical("Periodic cleanup failed too many times. Stopping background task.")
                break
...
async def on_startup():
    ...
    logger.info("🚀 CYBERPUNK CORE PRO v5.1 started successfully!")
