"""
╔══════════════════════════════════════════════════════════════════════════════╗
║           🔥 CYBERPUNK DISTRIBUTOR CORE v5.4 PRO EDITION 🔥                 ║
║                                                                              ║
...
logger.info("╔═══════════════════════════════════════════════════════════════╗")
logger.info("║  🔥 CYBERPUNK DISTRIBUTOR CORE v5.4 PRO - INITIALIZING... 🔥  ║")
logger.info("╚═══════════════════════════════════════════════════════════════╝")
...
def safe_md(text: str) -> str:
    """إزالة كل أحرف Markdown الخاصة لمنع كسر الرسالة"""
    if not text:
        return ""
    t = str(text)
    for ch in ['*', '_', '`', '[', ']', '(', ')', '~', '>', '#', '+', '-', '=', '|', '{', '}', '.', '!']:
...
async def tg_send_html(chat_id: str, text: str, **kwargs):
    """إرسال رسالة بصيغة HTML — أكثر أماناً من Markdown"""
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
...
async def tg_edit_html(chat_id: str, message_id: int, text: str, **kwargs):
    """تعديل رسالة بصيغة HTML — أكثر أماناً من Markdown"""
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/editMessageText"
    payload = {
        "chat_id": chat_id,
        "message_id": message_id,
    ...
    async with httpx.AsyncClient(timeout=10.0) as c:
        try:
            ...
            logger.error(f"tg_edit_html failed: {e}")
...
def format_stats_message(db) -> str:
    ...
    lines = [
        ...
        "╠═══════════════════════════════════════╣",
        f"║  📦 إجمالي الحسابات:    <code>{total:>6}</code> ║",
        f"║  🟢 الجاهزة:            <code>{avail:>6}</code> ║",
        f"║  🔴 الموزعة:            <code>{given:>6}</code> ║",
        f"║  👥 إجمالي المستخدمين:  <code>{total_users:>6}</code> ║",
        f"║  🚫 المحظورون:          <code>{banned:>6}</code> ║",
        f"║  ⏳ الحجوزات النشطة:    <code>{active_res:>6}</code> ║",
        f"║  📈 سحوبات اليوم:       <code>{claims_today:>6}</code> ║",
        "╠═══════════════════════════════════════╣",
    ...
    for cfg, count in top_configs:
        ...
        lines.append(f"║     • <code>{escape_html(display)}</code> {count:>3} ║")
...
def format_user_history(db, user_id: str) -> str:
    ...
    lines = [
        f"🕐 「 سجل عملياتي 」 🕐\n",
        f"👤 المستخدم: <code>{escape_html(user.user_id)}</code>",
        f"📅 تاريخ التسجيل: <code>{format_datetime(user.join_date)}</code>",
        f"🏆 السمعة: <code>{user.reputation_score}/100</code>",
        f"📊 إجمالي السحوبات: <code>{user.total_claims}</code>\n",
        "آخر السحوبات:",
        "━━━━━━━━━━━━━━━━━━━━",
    ]

    if not deliveries:
        lines.append("لا توجد سحوبات مسجلة.")
    else:
        for d in deliveries:
            lines.append(f"📦 <code>{escape_html(d.config_name or 'غير معروف')}</code>")
...
def format_reservation_message(db, user_id: str) -> str:
    ...
    for res in reservations:
        ...
        lines.append(f"📦 <code>{escape_html(res.config_name)}</code>")
        lines.append(f"   ⏳ متبقي: <code>{mins} دقيقة</code>")
        lines.append(f"   📅 ينتهي: <code>{format_datetime(res.expires_at)}</code>")
        lines.append("   ━━━")
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        if "callback_query" in payload:
            ...
            if data.startswith("claim_cfg:"):
                try:
                    ...
                    # ★ حذف رسالة الاختيار وإرسال رسالة جديدة بـ HTML
                    await tg_delete_message(chat_id, message_id)

                    clean_config = escape_html(account.config_name)
                    clean_data = escape_html(account.account_data)
                    clean_captured = escape_html(account.captured_data)

                    await tg_send_html(chat_id,
                        f"🌌 <b>⚡ 「 تم السحب بنجاح 」 ⚡</b> 🌌\n\n"
                        f"📦 <b>النوع:</b> <code>{clean_config}</code>\n\n"
                        f"👤 <b>الحساب:</b>\n<code>{clean_data}</code>\n\n"
                        f"⚙️ <b>المستخرج:</b>\n<code>{clean_captured}</code>\n\n"
                        f"🔒 <i>STATUS: DELIVERED</i>\n"
            ...
            if data.startswith("confirm_reserve:"):
                try:
                    ...
                    await tg_edit_html(chat_id, message_id,
                        f"✅ <b>تم الحجز بنجاح!</b>\n\n"
                        f"📦 النوع: <code>{escape_html(selected)}</code>\n"
                        f"⏳ مدة الحجز: <code>{RESERVATION_MINUTES}</code> دقيقة\n"
                        f"📅 ينتهي: <code>{format_datetime(expires)}</code>\n\n"
                        f"<i>اضغط \"تأكيد السحب\" لاستلام الحساب.</i>",
            ...
            if data == "reset_delivered" and is_admin:
                try:
                    ...
                    await tg_edit_html(chat_id, message_id,
                        f"🧹 <b>تم تصفير سجل التوزيع</b>\n\n"
                        f"📊 تم حذف <code>{count}</code> سجل",
            ...
            if data == "clear_accounts" and is_admin:
                try:
                    ...
                    await tg_edit_html(chat_id, message_id,
                        f"🚨 <b>تم تصفير المخزن بالكامل</b>\n\n"
                        f"📦 تم حذف <code>{count}</code> حساب",
                        reply_markup=get_admin_keyboard()
                    )
                except Exception as e:
                    db.rollback()
            ...
            if data == "refresh_admin_stats" and is_admin:
                try:
                    ...
                    await tg_edit_html(chat_id, message_id, stats_msg,
                                      reply_markup=get_admin_keyboard())
                except Exception as e:
            ...
            if data == "detailed_report" and is_admin:
                try:
                    ...
                    await tg_edit_html(chat_id, message_id, report,
                                      reply_markup=get_admin_keyboard())
                except Exception as e:
            ...
            if data == "scan_inventory" and is_admin:
                try:
                    ...
                    status_text = "✅ سليم" if orphaned_fixed == 0 else "🔧 تم الإصلاح"
                    msg = (
                        f"🔍 <b>نتيجة فحص المخزن</b>\n\n"
                        f"📦 الإجمالي: <code>{total}</code>\n"
                        f"🟢 متاح: <code>{avail}</code>\n"
                        f"🔴 موزع: <code>{given}</code>\n"
                        f"⏳ محجوز: <code>{reserved}</code>\n"
                        f"⚠️ يتيم (تم إصلاحه): <code>{orphaned_fixed}</code>\n\n"
                        f"<i>حالة المخزن: {status_text}</i>"
                    )
                    await tg_edit_html(chat_id, message_id, msg,
                                      reply_markup=get_admin_keyboard())
            ...
            if data == "banned_users" and is_admin:
                try:
                    ...
                    if not bans:
                        msg = "✅ <b>لا يوجد مستخدمون محظورون حالياً.</b>"
                    else:
                        lines = ["⚠️ <b>قائمة المحظورين النشطين:</b>\n"]
                        for b in bans:
                            exp = f" (ينتهي: {format_datetime(b.expires_at)})" if b.expires_at else " (دائم)"
                            lines.append(
                                f"🚫 <code>{escape_html(b.user_id)}</code>\n"
                                f"   السبب: <code>{escape_html(b.reason or 'غير محدد')}</code>\n"
                                f"   بتاريخ: <code>{format_datetime(b.banned_at)}</code>{exp}\n"
                                f"   ━━━"
                            )
                        msg = "\n".join(lines)
            ...
            if data == "broadcast" and is_admin:
                ...
                await tg_edit_html(chat_id, message_id,
                    ...
                    "<i>سيتم إرسالها لجميع المستخدمين.</i>",
            ...
            if data == "settings" and is_admin:
                notif_status = "مفعّل" if NOTIFY_ADMINS_ON_LOW_STOCK else "معطّل"
                msg = (
                    "⚙️ <b>الإعدادات الحالية</b>\n\n"
                    f"⏳ مدة الانتظار بين السحوبات: <code>{CLAIM_COOLDOWN_SECONDS // 3600}</code> ساعة\n"
                    f"🏦 مدة الحجز: <code>{RESERVATION_MINUTES}</code> دقيقة\n"
                    f"📊 حد السحوبات: <code>{RATE_LIMIT_CLAIMS}</code> / ساعة\n"
                    f"⚠️ حد التنبيه المنخفض: <code>{LOW_STOCK_THRESHOLD}</code> حساب\n"
                    f"🔔 تنبيه المخزن: <code>{notif_status}</code>\n\n"
                    "<i>لتعديل: غيّر متغيرات البيئة في Render.</i>"
                )
                await tg_edit_html(chat_id, message_id, msg,
                              reply_markup=get_admin_keyboard())
            ...
            if data == "ban_user" and is_admin:
                ...
                await tg_edit_html(chat_id, message_id,
                    ...
                    "<i>مثال: <code>123456789 | سبب</code></i>\n"
                    "<i>أو أرسل المعرف فقط لحظر بدون سبب.</i>",
                    reply_markup=get_cancel_keyboard()
                )
            ...
            if data == "list_banned" and is_admin:
                try:
                    ...
                    if not bans:
                        msg = "✅ <b>لا يوجد محظورون.</b>"
                    else:
                        lines = ["🚫 <b>المحظورون:</b>\n"]
                        for b in bans:
                            lines.append(f"• <code>{escape_html(b.user_id)}</code> — {escape_html(b.reason or '?')}")
                        msg = "\n".join(lines)
                    await tg_edit_html(chat_id, message_id, msg,
            ...
            if data == "list_users" and is_admin:
                try:
                    ...
                    if not users:
                        msg = "👥 <b>لا يوجد مستخدمون مسجلون.</b>"
                    else:
                        lines = ["👥 <b>المستخدمون (آخر 15):</b>\n"]
                        for u in users:
                            name = escape_html(u.first_name or u.username or "بدون اسم")
                            lines.append(
                                f"• <code>{escape_html(u.user_id)}</code> {name}\n"
            ...
            if data == "user_stats" and is_admin:
                try:
                    ...
                    lines = [
                        ...
                        f"👥 الإجمالي: <code>{total_users}</code>",
                        f"📦 إجمالي السحوبات: <code>{total_claims}</code>",
                        f"📈 متوسط السحوبات/مستخدم: <code>{avg_claims:.1f}</code>\n",
                        "<b>🏆 أكثر 5 مستخدمين نشاطاً:</b>",
                    ]
                    for u in top_users:
                        name = escape_html(u.first_name or u.username or u.user_id)
                        lines.append(f"  • {name}: <code>{u.total_claims}</code> سحب")
        ...
        # ========== TEXT MESSAGES ==========
            ...
            if text == "/start":
                welcome = (
                    "🌌 <b>⚡ CYBERPUNK DISTRIBUTOR ⚡</b> 🌌\n\n"
                    "مرحباً بك في نظام التوزيع المتقدم.\n\n"
                    "⚡ <b>سحب حساب جديد</b> — احصل على حساب فوراً\n"
                    "📡 <b>إحصائيات المخزن</b> — عرض حالة المخزون\n"
                    "🤖 <b>عمليات أوبن بلوت</b> — مراقبة العمليات الحية\n"
                    "📊 <b>إحصائيات متقدمة</b> — تحليلات مفصلة\n"
                    "🕐 <b>سجل عملياتي</b> — سجل سحوباتك\n"
                    "🏦 <b>حجز مؤقت</b> — احجز حساباً مؤقتاً\n"
                )
                await tg_send_html(chat_id, welcome,
            ...
            if text == "⚡ 🧬 سحب حساب جديد 🧬 ⚡":
                try:
                    ...
                    if banned:
                        await tg_send_html(chat_id,
                            f"🚫 <b>أنت محظور!</b>\nالسبب: <code>{escape_html(reason)}</code>")
                        return {"status": "ok"}
                    ...
                    if not configs:
                        await tg_send_html(chat_id,
                            "😔 <b>المخزن فارغ حالياً</b>\n\n"
                            "<i>لا توجد حسابات متاحة. جرب لاحقاً.</i>")
                        return {"status": "ok"}
                    ...
                    total_avail = sum(c for _, c in configs)
                    await tg_send_html(chat_id,
                        f"⚡ <b>「 اختر نوع الحساب 」</b> ⚡\n\n"
                        f"🟢 <code>{total_avail}</code> حساب متاح\n"
                        f"📦 <code>{len(configs)}</code> أنواع مختلفة",
                        reply_markup={"inline_keyboard": buttons}
                    )
                except Exception as e:
                    logger.error(f"Claim menu error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ حدث خطأ، حاول مرة أخرى")
                return {"status": "ok"}
            ...
            if text == "📡 🌐 إحصائيات المخزن 🌐 📡":
                try:
                    stats_msg = format_stats_message(db)
                    await tg_send_html(chat_id, stats_msg)
                except Exception as e:
                    logger.error(f"Stats error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ خطأ في تحميل الإحصائيات")
                return {"status": "ok"}
            ...
            if text == "🤖 ⚔️ عمليات أوبن بلوت الجارية ⚔️ 🤖":
                try:
                    ...
                    await tg_send_html(chat_id, "❌ خطأ في الاتصال بـ OpenBullet")
                return {"status": "ok"}
            ...
            if text == "📊 📈 إحصائيات متقدمة 📈 📊":
                try:
                    report = format_detailed_report(db)
                    await tg_send_html(chat_id, report)
                except Exception as e:
                    logger.error(f"Advanced stats error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ خطأ في تحميل التقرير")
                return {"status": "ok"}
            ...
            if text == "🕐 📜 سجل عملياتي 📜 🕐":
                try:
                    ...
                    await tg_send_html(chat_id, history)
                except Exception as e:
                    logger.error(f"History error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ خطأ في تحميل السجل")
                return {"status": "ok"}
            ...
            if text == "🏦 ⏳ حجز مؤقت ⏳ 🏦":
                try:
                    ...
                    if configs:
                        ...
                        await tg_send_html(chat_id, res_msg,
                                      reply_markup={"inline_keyboard": buttons})
                    else:
                        await tg_send_html(chat_id,
                            res_msg + "\n\n<i>لا توجد حسابات متاحة للحجز.</i>")
                except Exception as e:
                    logger.error(f"Reservation error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ خطأ في نظام الحجز")
                return {"status": "ok"}
            ...
            if text == "🛠️ 👾 لوحة تحكم المطور 👾 🛠️" and is_admin:
                try:
                    ...
                    await tg_send_html(chat_id, "❌ خطأ في فتح لوحة التحكم")
                return {"status": "ok"}
            ...
            if chat_id in pending_broadcasts:
                ...
                try:
                    ...
                    for u in users:
                        try:
                            await tg_send_html(u.user_id, text)
                            sent += 1
                        except Exception:
                            failed += 1

                    await tg_send_html(chat_id,
                        f"📢 <b>نتيجة الإشعار العام</b>\n\n"
                        f"✅ تم الإرسال: <code>{sent}</code>\n"
                        f"❌ فشل: <code>{failed}</code>\n"
                        f"📊 الإجمالي: <code>{len(users)}</code>")
                    log_audit(db, chat_id, "broadcast",
                              f"Sent to {sent} users, {failed} failed", request)
                except Exception as e:
                    logger.error(f"Broadcast error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ حدث خطأ في الإرسال")
                return {"status": "ok"}
            ...
            if chat_id in pending_bans:
                ...
                try:
                    ...
                    await tg_send_html(chat_id,
                        ...
                        f"👤 المعرف: <code>{escape_html(uid)}</code>\n"
                        f"📝 السبب: <code>{escape_html(reason)}</code>")
                    log_audit(db, chat_id, "ban_user",
                              f"Banned {uid}: {reason}", request)
                except Exception as e:
                    db.rollback()
                    logger.error(f"Ban error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ حدث خطأ في الحظر")
                return {"status": "ok"}
            ...
            if chat_id in pending_unbans:
                ...
                try:
                    ...
                    if ban:
                        ...
                        await tg_send_html(chat_id,
                            f"✅ <b>تم فك حظر</b>\n\n"
                            f"👤 المعرف: <code>{escape_html(target_id)}</code>")
                        log_audit(db, chat_id, "unban_user",
                                  f"Unbanned {target_id}", request)
                    else:
                        await tg_send_html(chat_id,
                            f"❌ <b>المستخدم غير محظور</b>\n\n"
                            f"👤 <code>{escape_html(target_id)}</code>")
                except Exception as e:
                    db.rollback()
                    logger.error(f"Unban error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ حدث خطأ في فك الحظر")
                return {"status": "ok"}
                ...
                try:
                    ...
                    if not user_obj:
                        ...
                        await tg_send_html(chat_id,
                            ...
                            f"🆔 المعرف: <code>{escape_html(user_obj.user_id)}</code>\n"
                            f"📛 username: <code>{escape_html(user_obj.username or 'N/A')}</code>\n"
                            f"📊 السحوبات: <code>{user_obj.total_claims}</code>\n"
                            f"🏆 السمعة: <code>{user_obj.reputation_score}/100</code>\n"
                            f"🔒 الحالة: <code>{ban_status}</code>\n"
                            f"📅 التسجيل: <code>{format_datetime(user_obj.join_date)}</code>\n"
                            f"🕐 آخر نشاط: <code>{time_ago(user_obj.last_activity)}</code>")
                except Exception as e:
                    logger.error(f"Search error: {e}", exc_info=True)
                    await tg_send_html(chat_id, "❌ حدث خطأ في البحث")
                return {"status": "ok"}
...
async def startup_event():
    logger.info("🚀 CYBERPUNK DISTRIBUTOR CORE v5.4 — READY")
    logger.info("🔗 Webhook URL: /webhook/telegram")
    logger.info("🔗 Hit Receiver: /webhook/hit")
